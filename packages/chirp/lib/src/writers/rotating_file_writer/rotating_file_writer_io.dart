...
/// Matches the timestamp portion of a rotated log filename.
///
/// Generated by [RotatingFileWriterIo._generateRotatedPath]:
///   `baseName.YYYY-MM-DD_HH-MM-SS[_N].ext[.gz]`
///
/// After stripping the `baseName.` prefix, the remainder must start with
/// a full timestamp like `2024-01-15_10-30-45`, optionally followed by
/// a counter `_1`, then the extension (and optionally `.gz`).
final _rotatedTimestampPattern =
    RegExp(r'^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}(_\d+)?');

/// Whether [fileName] is a rotated log file for the given [baseName].
///
/// Shared between writer (retention policy) and reader (file listing).
