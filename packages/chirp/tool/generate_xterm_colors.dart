...
void main() {
  ...
  buffer.writeln();
  buffer
      .writeln('/// The 256-color xterm palette as [IndexedColor] constants.');
  buffer.writeln('///');
  buffer.writeln(
      '/// Codes 0-15 are standard ANSI colors (prefer [Ansi16] for these).');
  buffer.writeln('/// Codes 16-231 are a 6x6x6 color cube.');
  buffer.writeln('/// Codes 232-255 are a grayscale ramp.');
  buffer.writeln('///');
  buffer.writeln('/// ## Example');
  buffer.writeln('///');
  buffer.writeln('/// ```dart');
  buffer.writeln('/// buffer.pushStyle(foreground: Ansi256.orange1_214);');
  buffer.writeln('/// buffer.write("Warning!");');
  buffer.writeln('/// buffer.popStyle();');
  buffer.writeln('/// ```');
  buffer.writeln('abstract final class Ansi256 {');
  ...
  for (var i = 0; i < 256; i++) {
    ...
    buffer.writeln('  static const ${color.name} = IndexedColor($i);');
  }

  buffer.writeln('}');

  final outputFile = File('lib/src/ansi/ansi256.g.dart');
...
const _xtermColorData = <int, (String, String)>{
  0: ('Black', '#000000'),
  1: ('Maroon', '#800000'),
  2: ('Green', '#008000'),
  3: ('Olive', '#808000'),
...
/// Standard color names (0-15) matching traditional terminal naming.
const _standardColorNames = <int, String>{
  0: 'black',
  1: 'red',
  2: 'green',
  3: 'yellow',
...
_ColorInfo _getXtermColor(int code) {
  final data = _xtermColorData[code]!;
  final displayName = data.$1;
  final hex = data.$2;
  ...
  // Generate enum name: colorName + code number
  String enumName;
  if (code < 16) {
    // Standard colors: use traditional names + code
  ...
  return _ColorInfo(enumName, displayName, hex, r, g, b);
}
